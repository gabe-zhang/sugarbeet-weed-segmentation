...
class CrossEntropy(nn.Module):
    ...
    def forward(
        ...
        if self.weights is not None:
            ...
            ).permute(0, 3, 1, 2)  # [N x n_classes x H x W]
            weights_gathered = weight_matrix[
                target_one_hot
            ]  # M = N * (H * W) entries
...
def kl_div_loss(
    x_logits_pred: torch.Tensor,
    x_logits_true: torch.Tensor,
    mask_keep: Optional[torch.Tensor] = None,
) -> torch.Tensor:
...
def to_one_hot(tensor: torch.Tensor, n_classes: int) -> torch.Tensor:
    ...
    if len(tensor.size()) == 1:
        ...
        if tensor.is_cuda:
            one_hot = torch.zeros(
                b, n_classes, device=torch.device("cuda")
            ).scatter_(1, tensor.unsqueeze(1), 1)
        else:
            one_hot = torch.zeros(b, n_classes).scatter_(
        ...
        if tensor.is_cuda:
            one_hot = torch.zeros(
                n, n_classes, b, device=torch.device("cuda")
            ).scatter_(1, tensor.unsqueeze(1), 1)
        else:
            one_hot = torch.zeros(n, n_classes, b).scatter_(
                1, tensor.unsqueeze(1), 1
            )
        ...
        if tensor.is_cuda:
            one_hot = torch.zeros(
                n, n_classes, h, w, device=torch.device("cuda")
            ).scatter_(1, tensor.unsqueeze(1), 1)
        else:
