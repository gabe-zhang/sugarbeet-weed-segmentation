...
class non_bottleneck_1d(nn.Module):
    ...
    def forward(
        self, input, apply_relu: bool = True, apply_instance_norm: bool = False
    ):
...
class ERFNetModel(nn.Module):
    def __init__(self, num_classes: int, pretrained: bool = False):
        ...
        if pretrained:
            ...
            ckpt_dict = torch.load(local_file, weights_only=False)
    ...
    def forward_ibn_net(self, img: torch.Tensor) -> torch.Tensor:
        # ---------------- ENCODER ----------------
        img = self.downsampler_block_01.forward(
            img, apply_batch_norm=False, apply_instance_norm=True
        )
        img = self.downsampler_block_02.forward(
            img, apply_batch_norm=False, apply_instance_norm=True
        )
