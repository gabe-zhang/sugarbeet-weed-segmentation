...
class RandomGlobalSaturation(MyColorAugmentation):
    ...
    def __call__(self, image: torch.Tensor) -> torch.Tensor:
        ...
        if random.random() <= self.prob:
            rand_saturation_factor = random.uniform(
                self.min_saturation_factor, self.max_saturation_factor
            )
            image = functional.adjust_saturation(
                image, saturation_factor=rand_saturation_factor
            )
...
class RandomGlobalHue(MyColorAugmentation):
    def __init__(
        self,
        min_hue_factor: float = 0.0,
        max_hue_factor: float = 0.0,
        prob: float = 0.5,
    ):
    ...
    def __call__(self, image: torch.Tensor) -> torch.Tensor:
        """See: https://pytorch.org/vision/0.8/transforms.html#functional-transforms"""
        if random.random() <= self.prob:
            rand_hue_factor = random.uniform(
                self.min_hue_factor, self.max_hue_factor
            )
...
class RandomGlobalBrightness(MyColorAugmentation):
    def __init__(
        self,
        min_brightness_factor: float = 1,
        max_brightness_factor: float = 1,
        prob: float = 0.5,
    ):
    ...
    def __call__(self, image: torch.Tensor) -> torch.Tensor:
        """See: https://pytorch.org/vision/0.8/transforms.html#functional-transforms"""
        if random.random() <= self.prob:
            rand_brightness_factor = random.uniform(
                self.min_brightness_factor, self.max_brightness_factor
            )
            image = functional.adjust_brightness(
                image, brightness_factor=rand_brightness_factor
            )
...
class RandomGlobalContrast(MyColorAugmentation):
    def __init__(
        self,
        min_contrast_factor: float = 1,
        max_contrast_factor: float = 1,
        prob: float = 0.5,
    ):
    ...
    def __call__(self, image: torch.Tensor) -> torch.Tensor:
        if random.random() <= self.prob:
            image_gaussian_noise = (
                torch.randn(image.shape) * self.std
            ) + self.mean
...
class AugMixAugmentator(MyColorAugmentation):
    ...
    def __init__(self):
        ...
        self.aug_prob_coeff: float = (
            1.0  # Probability distribution coefficients
        )
    ...
    def __call__(self, image: torch.Tensor) -> torch.Tensor:
        ...
        """
        aug_list = (
            augmentations_augmix.augmentations_all
...
def get_color_augmentations(cfg, stage: str) -> List[Callable]:
    ...
    for color_aug_name in cfg[stage]["color_data_augmentations"].keys():
        if color_aug_name == "random_global_brightness":
            min_brightness_factor = cfg[stage]["color_data_augmentations"][
                color_aug_name
            ]["min_brightness_factor"]
            max_brightness_factor = cfg[stage]["color_data_augmentations"][
                color_aug_name
            ]["max_brightness_factor"]
            augmentor = RandomGlobalBrightness(
                min_brightness_factor, max_brightness_factor
            )
            color_augmentations.append(augmentor)

        if color_aug_name == "random_global_contrast":
            min_contrast_factor = cfg[stage]["color_data_augmentations"][
                color_aug_name
            ]["min_contrast_factor"]
            max_contrast_factor = cfg[stage]["color_data_augmentations"][
                color_aug_name
            ]["max_contrast_factor"]
            augmentor = RandomGlobalContrast(
                min_contrast_factor, max_contrast_factor
            )
            color_augmentations.append(augmentor)

        if color_aug_name == "random_global_saturation":
            min_saturation_factor = cfg[stage]["color_data_augmentations"][
                color_aug_name
            ]["min_saturation_factor"]
            max_saturation_factor = cfg[stage]["color_data_augmentations"][
                color_aug_name
            ]["max_saturation_factor"]
            augmentor = RandomGlobalSaturation(
                min_saturation_factor, max_saturation_factor
            )
            color_augmentations.append(augmentor)

        if color_aug_name == "random_gaussian_color_jitter":
            mean = cfg[stage]["color_data_augmentations"][color_aug_name][
                "mean"
            ]
        ...
        if color_aug_name == "random_global_hue":
            min_hue_factor = cfg[stage]["color_data_augmentations"][
                color_aug_name
            ]["min_hue_factor"]
            max_hue_factor = cfg[stage]["color_data_augmentations"][
                color_aug_name
            ]["max_hue_factor"]
    ...
    assert len(color_augmentations) == len(
        cfg[stage]["color_data_augmentations"].keys()
    )
