...
def image_rgb_stack(
    r_chan: torch.Tensor, g_chan: torch.Tensor, b_chan: torch.Tensor
) -> torch.Tensor:
...
def check_split_file(path_to_split_file: str) -> bool:
    ...
    with open(path_to_split_file, "r") as istream:
        ...
        no_intersection = (
            (len(train_test_intersection) == 0)
            and (len(train_val_intersection) == 0)
            and (len(val_test_intersection) == 0)
        )
