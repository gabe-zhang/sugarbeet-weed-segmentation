...
class GlobalImageNormalizer(ImageNormalizer):
    ...
    def __init__(self, rgb_means: List[float], rgb_stds: List[float]) -> None:
        ...
        self.normalizer = transforms.Normalize(
            mean=self.rgb_means, std=self.rgb_stds
        )
...
class SingleImageNormalizer(ImageNormalizer):
    ...
    def transform_torch_img_channel(
        self, channel: torch.Tensor, mean: float, std: float
    ):
        """Compute z-score."""
    ...
    def normalize(self, image: torch.Tensor) -> torch.Tensor:
        ...
        image_normalized = common.image_rgb_stack(
            r_chan_normed, g_chan_normed, b_chan_normed
        )
