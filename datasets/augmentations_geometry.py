...
class ResizeTransform(GeometricDataAugmentation):
    ...
    def __call__(self, image: torch.Tensor) -> torch.Tensor:
        resized_img = functional.resize(image, size=[self.h, self.w], interpolation=transforms.InterpolationMode.BILINEAR)
        return resized_img
...
class MyPaddingTransform(GeometricDataAugmentation):
    ...
    def __call__(self, image: torch.Tensor) -> torch.Tensor:
        ...
        padded_img = functional.pad(image, (0, pad_w, 0, pad_h), 'constant', value=0)
...
def get_geometric_augmentations(cfg, stage: str) -> List[GeometricDataAugmentation]:
    ...
    for tf_name in cfg[stage]['geometric_data_augmentations'].keys():
        ...
        if tf_name == 'pad':
            padding_divisor = cfg[stage]['geometric_data_augmentations'][tf_name]['divisor']
