...
def blend_images(
    ...
    Args:
        ...
        alpha (float, optional): strength of blending for 1st image.
        Defaults to 0.5.
...
class BasicVisualizer(ABC):
    ...
    @abstractmethod
    def save_visualize_batch(
        self, path_to_dir: str, outputs: Dict[str, Any], batch: Dict[str, Any]
    ) -> None:
...
class WeedBboxVisualizer(BasicVisualizer):
    ...
    def save_visualize_batch(
        ...
        path_to_dir = os.path.join(path_to_dir, "weed_bboxes")
        ...
        with torch.no_grad():
            ...
            fnames = batch["fname"]
            for input_image_before_norm, logits_single_img, filename in zip(
                imgs, logits, fnames
            ):
...
class SemanticsOverlayCorrectIncorrectVisualizer(BasicVisualizer):
    ...
    def save_visualize_batch(
        ...
        path_to_dir = os.path.join(
            path_to_dir, "semantic_overlays_correct_incorrect"
        )
        if not os.path.exists(path_to_dir):
            os.makedirs(path_to_dir)
        ...
        with torch.no_grad():
            ...
            fnames = batch["fname"]
            for (
                input_image_before_norm,
                logits_single_img,
                anno,
                filename,
            ) in zip(imgs, logits, annos, fnames):
                pred = torch.argmax(logits_single_img, dim=0)
                ...
                for class_id, class_color in self.classes_to_colors.items():
                    ...
                    color_correct_tensor = (
                        torch.Tensor(class_color)
                        .unsqueeze(1)
                        .to(canvas.device)
                    )
...
def get_visualizers(cfg: Dict) -> List[BasicVisualizer]:
    ...
    for visualizer_name in cfg["visualizers"].keys():
        if visualizer_name == "weed_bbox_visualizer":
            visualizer = WeedBboxVisualizer()
            visualizers.append(visualizer)

        elif visualizer_name == "input_image_visualizer":
            visualizer = InputImageVisualizer()
            visualizers.append(visualizer)

        elif visualizer_name == "semantic_map_visualizer":
        ...
        elif visualizer_name == "semantic_overlay_visualizer":
            classes_to_colors = cfg["visualizers"][visualizer_name][
                "classes_to_colors"
            ]
        ...
        elif visualizer_name == "ground_truth_visualizer":
            classes_to_colors = cfg["visualizers"][visualizer_name][
                "classes_to_colors"
            ]
            ...
            visualizer = SemanticsOverlayCorrectIncorrectVisualizer(
                classes_to_colors
            )
...
class VisualizerCallback(Callback):
    ...
    def __init__(
        self,
        visualizers: List[BasicVisualizer],
        vis_train_every_x_epochs: int = 1,
        vis_val_every_x_epochs: int = 1,
    ):
        """Constructor.

        Args:
            vis_train_every_x_epochs (int): Frequency of train visualizations.
                Defaults to 1.
            vis_val_every_x_epochs (int): Frequency of val visualizations.
                Defaults to 1.
        """
        super().__init__()
