...
class Postprocessor(ABC):
    ...
    @abstractmethod
    def save_postprocessed_logits(
        self,
        processed_logits: Union[torch.Tensor, None],
        path_to_dir: str,
        fnames: List[str],
    ) -> None:
...
class ProbablisticSoftmaxPostprocessor(Postprocessor):
    ...
    def process_logits(self, logits: torch.Tensor) -> torch.Tensor:
        ...
        softmax_probs = functional.softmax(
            logits, dim=1
        )  # [batch_size x num_classes x H x W]
...
class PostprocessorrCallback(Callback):
    ...
    def __init__(
        ...
        postprocess_train_every_x_epochs: int = 1,
        ...
        self.postprocess_train_every_x_epochs = (
            postprocess_train_every_x_epochs
        )
    ...
    def on_train_batch_end(
        ...
        if (epoch % self.postprocess_train_every_x_epochs) == 0 and (
            epoch != 0
        ):
            path = os.path.join(
                trainer.log_dir, "train", "postprocess", f"epoch-{epoch:06d}"
            )

            for postprocessor in self.postprocessors:
                # process logits
                processed_logits = postprocessor.process_logits(
                    outputs["logits"]
                )
    ...
    def on_validation_batch_end(
        ...
        # visualize
        epoch = trainer.current_epoch
        if ((epoch + 1) % self.postprocess_val_every_x_epochs) == 0 and (
            epoch != 0
        ):
            path = os.path.join(
                trainer.log_dir, "val", "postprocess", f"epoch-{epoch:06d}"
            )
