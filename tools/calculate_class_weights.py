...
def parse_args() -> Dict[str, str]:
    ...
    parser.add_argument(
        ...
        required=True,
        type=str,
        help="Path to Directory to get the annotations from.",
    )
...
if __name__ == "__main__":
    ...
    filenames = [
        fname
        for fname in os.listdir(args["anno_dir"])
    ...
    total = np.sum(
        [
            total_class_frequencies[class_id]
            for class_id in total_class_frequencies.keys()
        ]
    )

    relativ_class_frequencies = {}
    for class_id, class_frequency in total_class_frequencies.items():
        relativ_class_frequencies[class_id] = (
            total_class_frequencies[class_id] / total
        )
