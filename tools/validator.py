...
# MIT License
#
# Copyright (c) 2023 Jan Weyler, Federico Magistri, Elias Marks,
# Yue Linn Chong, Matteo Sodano, Gianmarco Roggiolani,
# Nived Chebrolu, Cyrill Stachniss, Jens Behley.
#
# Adapted from phenobench-baselines.
    ...
    try:
        ...
        if not FLAGS.zipfile.endswith(".zip"):
            raise ValidationException(
                'Competition submission must end with ".zip"'
            )
        print(checkmark)
        ...
        with ZipFile(FLAGS.zipfile) as zipfile:
            ...
            image_files = sorted(
                os.listdir(
                    os.path.join(FLAGS.phenobench_dir, "test", "images")
                )
            )
                ...
                if "bboxes" in expected_folder:
                    for image_file in image_files:
                        ...
                        if expected_file not in prediction_files:
                            raise ValidationException(
                                f"Missing prediction for {image_file} in folder {expected_folder}!"
                            )
                else:
                    for image_file in image_files:
